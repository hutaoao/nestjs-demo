...
export class ApiKeyGuard implements CanActivate {
  ...
  // context: 当前请求上下文——这就是 ExecutionContext 的实际使用
  // ExecutionContext 可以拿到：
  //   context.getHandler()        → 当前调用的 Controller 方法
  //   context.getClass()          → 当前调用的 Controller 类
  //   context.switchToHttp()      → HTTP 请求对象
  //   context.switchToWs()        → WebSocket 上下文（NestJS 也支持）
  //   context.switchToRpc()       → 微服务 RPC 上下文
  //   同一个 Guard 可以同时处理 HTTP、WebSocket、RPC 三种协议
