...
async function bootstrap() {
  ...
  // enableShutdownHooks() | 启用关闭钩子
  // 这行代码让 NestJS 监听系统关闭信号（SIGTERM/SIGINT），
  // 从而触发所有实现了 OnApplicationShutdown 的 Service 的 shutdown 方法
  // 不配这行的话，LifecycleDemoService.onApplicationShutdown() 不会执行
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription('极简博客 Demo — 覆盖 18 个 NestJS 核心概念')
    .setVersion('2.0')
    .addBearerAuth()  // ← 新增：在 Swagger UI 中添加 "Authorize" 按钮
    // 点击 Authorize 后输入 JWT Token，后续请求自动带上 Authorization 头
    .build();
